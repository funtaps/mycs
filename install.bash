...
cp ~/mycs/files/steam_appid.txt /opt/hlds/steam_appid.txt
cp ~/mycs/hlds_run.sh /bin/hlds_run.sh

# Add default config
cp ~/mycs/files/server.cfg /opt/hlds/cstrike/server.cfg

# Add maps
cp ~/mycs/maps/* /opt/hlds/cstrike/maps/
cp ~/mycs/files/mapcycle.txt /opt/hlds/cstrike/mapcycle.txt
...
cp ~/mycs/files/liblist.gam /opt/hlds/cstrike/liblist.gam
# Remove this line if you aren't going to install/use amxmodx and dproto
cp ~/mycs/files/plugins.ini /opt/hlds/cstrike/addons/metamod/plugins.ini

# Install dproto
mkdir -p /opt/hlds/cstrike/addons/dproto
cp ~/mycs/files/dproto_i386.so /opt/hlds/cstrike/addons/dproto/dproto_i386.so
cp ~/mycs/files/dproto.cfg /opt/hlds/cstrike/dproto.cfg
...
cp ~/mycs/files/maps.ini /opt/hlds/cstrike/addons/amxmodx/configs/maps.ini
